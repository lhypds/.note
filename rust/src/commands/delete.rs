...
pub fn main(_argv: &[String]) {
    ...
    let tmp_path = std::env::temp_dir().join(format!("note_delete_{}.txt", std::process::id()));
    ...
    match status {
        Ok(s) if s.success() && !selected.is_empty() => confirm_and_delete(&selected),
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
            eprintln!("fzf not found. Please install fzf to use the delete command.");
            std::process::exit(1);
        }
        _ => {}
    }
...
fn confirm_and_delete(path: &str) {
